...
interface EnterQueueNumberProps {
  ...
  onBack?: () => void;
  /** Pre-filled from the backend join response */
  suggestedNumber?: number;
}

export default function EnterQueueNumber({ institution, onSubmit, suggestedNumber }: EnterQueueNumberProps) {
  const [rawValue, setRawValue] = useState(suggestedNumber ? String(suggestedNumber) : "");
  ...
  const handleSubmit = () => {
    if (!rawValue.trim()) { setError("Please enter your queue number."); return; }
    if (!isValid) { setError("Enter a valid number between 1 and 9999."); return; }
    onSubmit(parsed);
  };
  ...
  return (
    <div style={{ maxWidth: 560, margin: "0 auto" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <h1 className="font-head" style={{ fontSize: "clamp(1.4rem, 3vw, 2rem)", fontWeight: 800, color: "var(--navy)", marginBottom: "0.4rem", lineHeight: 1.2 }}>
          Confirm your queue number
        </h1>
        <p style={{ color: "#6B82A8", fontSize: "0.9rem", lineHeight: 1.6 }}>
          {suggestedNumber
            ? `We assigned you #${suggestedNumber}. Verify this matches your physical ticket, or correct it below.`
      ...
      <div style={{ background: "white", border: "1.5px solid rgba(13,43,110,0.12)", borderRadius: 16, padding: "1.75rem", marginBottom: "1rem" }}>
        ...
        <div style={{ position: "relative", marginBottom: error ? "0.5rem" : "1.5rem" }}>
          <span style={{ position: "absolute", left: 18, top: "50%", transform: "translateY(-50%)", fontSize: "1.5rem", color: "#6B82A8", fontFamily: "var(--font-head)", fontWeight: 700, pointerEvents: "none" }}>
            #
          </span>
          <input
            type="text"
            ...
            maxLength={4}
            autoFocus
            style={{ width: "100%", paddingLeft: 44, paddingRight: 18, paddingTop: 16, paddingBottom: 16, borderRadius: 12, border: `2px solid ${error ? "#dc2626" : isValid ? "var(--sky)" : "rgba(13,43,110,0.14)"}`, background: "var(--off)", color: "var(--navy)", fontFamily: "var(--font-head)", fontWeight: 800, fontSize: "clamp(2rem, 6vw, 3rem)", outline: "none", boxSizing: "border-box", letterSpacing: "0.05em", transition: "border-color 0.2s" }}
            onFocus={(e) => { if (!error) e.currentTarget.style.borderColor = "var(--sky)"; }}
            onBlur={(e) => { if (!error && !isValid) e.currentTarget.style.borderColor = "rgba(13,43,110,0.14)"; }}
          />
        </div>
        ...
        {isValid && spotsAway !== null && (
          <div style={{ background: "var(--sky-pale)", border: "1px solid var(--sky-light)", borderRadius: 12, padding: "0.875rem 1rem", marginBottom: "1.5rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div>
        ...
        <button
          ...
          style={{ width: "100%", padding: "14px", borderRadius: 999, border: "none", background: rawValue.trim() ? "var(--navy)" : "#e2e8f0", color: rawValue.trim() ? "white" : "#94a3b8", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "1rem", cursor: rawValue.trim() ? "pointer" : "not-allowed", transition: "transform 0.15s, background 0.15s" }}
          onMouseEnter={(e) => { if (rawValue.trim()) { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.background = "var(--navy-mid)"; } }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.background = rawValue.trim() ? "var(--navy)" : "#e2e8f0"; }}
        >
          Start Tracking →
        </button>
      </div>
